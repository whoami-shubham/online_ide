...
var eqCursorPos = assert.deepEqual;
var eq = assert.equal;
var is = assert.ok;
...
isAce || testVim('i_overwrite_backspace', function(cm, vim, helpers) {
  cm.setCursor(0, 10);
  helpers.doKeys('i');
  helpers.doKeys('<Ins>');
  helpers.doInsertModeKeys('Backspace');
...
isAce || testVim('mark\'', function(cm, vim, helpers) {
  cm.setCursor(2, 2);
  cm.setCursor(0, 0);
  helpers.doKeys('`', '\'');
  helpers.assertCursorAt(2, 2);
...
testVim('zb_to_bottom', function(cm, vim, helpers){
  ...
  cm.setCursor(lineNum, 0);
  cm.ace.renderer.scrollCursorIntoView(); cm.refresh();
  helpers.doKeys('z', 'b');
...
isAce || testSubstitute('ex_substitute_dollar_match', {
  value: 'one,two \n three,four',
  expectedValue: 'one,two ,\n three,four',
  expr: '%s/$/,/g'});
isAce || testSubstitute('ex_substitute_newline_match', {
  value: 'one,two \n three,four',
